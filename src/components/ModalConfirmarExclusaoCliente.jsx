...
export default function ModalConfirmarExclusaoCliente({ isOpen, onClose, onConfirm, cliente }) {
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    console.log('🟢 ModalConfirmarExclusaoCliente - Props recebidas:', {
      isOpen,
      cliente,
      hasCliente: !!cliente
    });
  ...
  if (!isOpen || !cliente) {
    console.log('🔴 ModalConfirmarExclusaoCliente - Não renderizando:', {
      isOpen,
      cliente,
      reason: !isOpen ? 'isOpen é false' : 'cliente é null/undefined'
    });
    return null;
  }
  ...
  const handleConfirm = async () => {
    setIsLoading(true);
    try {
      await onConfirm(cliente.id, cliente.nome);
